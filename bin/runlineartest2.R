...
data_candidates=read.csv(x[1], header=T,  sep="\t")
# print(bla)
# Split data
...
for (i in seq(1, dim(result_prediction)[1])){
  result_prediction[i,3] = prop.test(result_prediction[i,1], nboot, p=0.5, alternative='greater')$p.value
}
...
result_prediction=cbind(result_prediction, blaref[,c("TM.score_TMalign_ref", "TM.score_TMalign_tneg")])
quality_pred = matrix(paste("Not_a_", type_prot[1], sep=""),  dim(blaref)[1], 1)
for (i in seq(1, dim(result_prediction)[1])){
  ...
  else if (result_prediction[i,3] <= 0.5 && result_prediction[i,4] > result_prediction[i,5] && result_prediction[i,4] >= 0.8){
    quality_pred[i] = paste("Likely_", type_prot[1], sep="")
  }
  else if (result_prediction[i,3] <= 0.5 && result_prediction[i,4] >= 0.5){
    quality_pred[i] = paste("Fair_", type_prot[1], sep="")
  }
}
result_prediction=cbind(rownames(result_prediction), result_prediction, quality_pred, as.character(blaref$Type))
colnames(result_prediction) = c("Sequence", type_prot, paste("Adjusted p.value of being a",type_prot[1]), "TM.score_TMalign_ref", "TM.score_TMalign_tneg", "Prediction", "Actual" )
write.table(result_prediction, file=x[5], sep="\t", quote=FALSE, row.names=FALSE)
...
#     blacandidate_dd=blacandidate[,c(5,7:13)]
    ...
    total_pred = rbind(total_pred, cbind(rownames(blacandidate_dd), rep(dtyp, dim(result_prediction)[1]), result_prediction, quality_pred))
    ##Herre
    #rownames(result_prediction) = rownames(blacandidate_dd)
    #rownames(result_prediction) = blacandidate[,1]
...
colnames(total_pred) = c("Sequence", "Type", type_prot, paste("Adjusted p.value of being a",type_prot[1]), "TM.score_TMalign_ref", "TM.score_TMalign_tneg", "Prediction")
#print(total_pred)
write.table(total_pred, file=paste(x[6], sep="") , sep="\t", quote=FALSE, row.names=FALSE)
