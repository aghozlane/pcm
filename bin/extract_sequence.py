...
def main():
    ...
    """
    if len(sys.argv) != 3:
      sys.exit("Error, the program take two arguments : ./{0} file.fasta output_dir".format(sys.argv[0]))
    #output_dir =  os.path.dirname(sys.argv[1]) + os.sep + ".".join(os.path.basename(sys.argv[1]).split(".")[:-1])
    output_dir = sys.argv[2] + os.sep
    print(output_dir)
    try:
      os.mkdir(output_dir)
    except:
