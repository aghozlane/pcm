...
input=function(){
        ...
        if(length(x) != 6){
                print("Usage : ./runlineartest.r prediction.csv universal_model.csv number_bootstrap reference_output.pdf ref_output.tsv prediction_output.tsv")
                q("yes",status=1)
        }
        return(x)
}
...
write.table(result_prediction, file=x[5], sep="\t", quote=FALSE)
...
#     blacandidate_dd=blacandidate[,c(5,7:13)]
    ...
    total_pred = rbind(total_pred, cbind(rownames(blacandidate_dd), rep(dtyp, dim(result_prediction)[1]), result_prediction))
    ##Herre
...
colnames(total_pred) = c("Sequence", "Type", type_prot)
