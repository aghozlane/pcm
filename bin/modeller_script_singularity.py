...
def get_arguments():
    ...
    parser.add_argument('-pr', dest='pdb_repository', type=isdir, 
                        ...
                        "(default /usr/local/bin/database/cleaned_pdb/).")
...
#                         "identification.")
    ...
    parser.add_argument('-psi', dest='run_psipred', action="store_true",
                        default=False, help='Run Psipred software.')
...
def run_checking(conf_data, summary_data, structure_check, path_check,
    ...
    for pdb in sorted(summary_data.iteritems(), key=lambda x: x[1][1]):
        ...
        if('proq_standalone' in structure_check):
            ...
            output_proq = "proq_" + ".".join(pdb[0].split(".")[:-1]) + ".txt"
            if psipred_file:
...
def main():
    ...
    # Run psipred
    if (args.run_psipred or args.path_psipred) and args.multifasta_file:
        print("Run psipred")
        if not args.path_psipred:
            args.path_psipred = ""
        args.psipred = run_secondary_structure_pred(conf_data,
                                                    ...
                                                    args.path_psipred,
                                                    args.thread)
    elif (args.run_psipred or args.path_psipred) and not args.multifasta_file:
